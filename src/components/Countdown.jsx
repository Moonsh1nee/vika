...
function FilledHeart() {
  return (
    <svg viewBox="0 0 24 24" className="h-8 w-8 text-red-500" aria-hidden="true">
      <path
        fill="currentColor"
...
function CalendarWidget({ weddingDate }) {
  ...
  return (
    <div className="border border-charcoal/20 bg-white px-6 py-5 md:px-8 md:py-6">
      <p
          ...
          <div key={i} className="flex h-8 items-center justify-center">
            {cell === null ? null : cell === day ? (
              <span className="relative flex items-center justify-center">
                <FilledHeart />
                <span className="absolute font-sans text-xs font-semibold text-white">
                  {cell}
                </span>
              </span>
            ) : (
              <span className="font-sans text-sm text-charcoal/80">{cell}</span>
            )}
          </div>
        ))}
      </div>
  ...
  return (
    <section className="section-frame bg-white px-6 py-20 md:py-28">
      <div className="mx-auto max-w-5xl">
        <h2 className="mb-16 text-center md:mb-20">
          <span
          ...
          <div className="flex shrink-0 flex-col items-center leading-none">
            {[wedding.date.day, monthNum, yearShort].map((val, i) => (
              <span
                ...
                style={{ fontSize: 'clamp(4.5rem, 13vw, 9rem)', lineHeight: 0.95 }}
              >
                {val}
              </span>
            ))}
